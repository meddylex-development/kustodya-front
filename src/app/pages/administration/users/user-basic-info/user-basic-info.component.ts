...
export class UserBasicInfoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // self.dataUser
    self.user_edit_id = parseInt(self.utilitiesService.fnGetSessionStorage('user_id'), 10);
  ...
  fnFormatDataUser(data_user) {
    ...
    const data_birth_date = (data_user['fechaNacimiento'] > 0) ? moment(data_user['fechaNacimiento']).format('DD-MM-YYYY') : '';
  ...
  fnCheckProfessionalDoctorState(state, document_type, document_number) {
    ...
    if (state) {
     if (document_type['id'] && document_number != null) {
      self.fnGetDoctorRethusByDNI(document_type['id'], parseInt(document_number), function(resp_doctor) {
